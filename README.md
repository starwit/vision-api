...
- Add the following to your `pyproject.toml` dependencies section\
    `visionapi = { git = "https://github.com/starwit/vision-api.git", subdirectory = "python/visionapi", tag = "3.8.0" }`
...
## Changelog
### 3.8.0
- Add `SamplingMetadata` to `SaeMessage` (records which detection sampler filters matched a forwarded message)
